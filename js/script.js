// event listeners for when the ajax requests are finished.
...
$.ajax({
  ...
  namesData.forEach(function (element, index) {
    ...
    if (j.getTime() < d.getTime() - 1.314e10) {
      active = "*";
    }
...
// creates the list of publish dates using a milisecond conversion, and decrementing by either week or average month time in. Then pushes those dates back into the correspondng dropdown.
function dateList(evt) {
  ...
  if (namesData[idx].updated === "WEEKLY") {
    ...
    for (i = newestDate; i >= oldestDate; i = i - 2.627942e9) {
      publishDates.push(i);
    }
  }
  publishDates.forEach(function (element) {
    ...
    $(`#dates`).append(
      `<option>${monthName[date.getMonth()]} ${ordinal_suffix_of(
        date.getDate() + 1
      )}, ${date.getFullYear()}</option>`
    );
  });
  $(`#dates`).append(
    `<option value="https://en.wikipedia.org/wiki/Category:The_New_York_Times_Best_Seller_list">(Explore further back in time on Wikipedia)</option>`
  )
}
...
// changes the date of the request in BASE_URL
$("#list-date").change(function () {
  $("#dates option:selected").each(function () {
    if ($(this).index() < publishDates.length) {
      ...
      BASE_URL =
        `https://api.nytimes.com/svc/books/v3/lists/${selcDate.getFullYear()}-${selcMonth}-${selcDay}/` +
        listIndex[idx] +
        `.json?`;
      handler();
    } else {
...
// ordinal suffix function for proper grammar in number lists. citation (https://stackoverflow.com/questions/13627308/add-st-nd-rd-and-th-ordinal-suffix-to-a-number/13627586)
  ...
  $.ajax({
    ...
    (data) => {
      ...
      currentList.forEach(function (element) {
        $(
          "ol"
        ).append(`<div class="animate__animated animate__flipInX list-item" ><div id="ranking">${element.rank}</div><div class="img-box"><div></div><a href="https://www.goodreads.com/search?q=${element.primary_isbn13}" target="_blank"><img src="${element.book_image}" class="book_image"></a></div>
